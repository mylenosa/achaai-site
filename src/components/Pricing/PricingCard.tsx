...
export const PricingCard: React.FC<PricingCardProps> = ({ plan, index }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 md:p-8 relative ${
        plan.highlighted ? 'ring-2 ring-emerald-500 scale-105' : ''
      } flex flex-col min-h-[500px] w-full text-gray-800 dark:text-gray-200`}
    >
      {plan.highlighted && (
